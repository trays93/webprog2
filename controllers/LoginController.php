    ...
    public function indexAction()
    {
        ...
        if (count($data) > 0) {
            ...
            try {
                ...
                $stmt = $conn->prepare("SELECT email, firstName, lastName, id, role FROM user WHERE email = :email AND password = :password");
                ...
                if ($stmt->rowCount() === 1) {
                    // siker
                    $result = $stmt->fetch();
                    $user = new User($result['email'], $result['firstName'], $result['lastName'], $result['id'], $result['role']);
                    $_SESSION['user'] = $user;
